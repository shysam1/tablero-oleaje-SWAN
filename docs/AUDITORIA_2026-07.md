...
### A1-1 · MEDIO — ✅ CORREGIDO (Cursor) — Percentiles de excedencia con NaN → anotaciones "P50 = nan m"
...
### A1-2 · MEDIO — ✅ CORREGIDO (Cursor) — `time` sin ordenar ni validar → productos silenciosamente erróneos
...
### A1-3 · MENOR — ✅ CORREGIDO (Cursor) — Rosa de oleaje: con Dir NaN los sectores no suman 100 %
...
### A1-4 · MENOR — ✅ CORREGIDO (Cursor) — Comparación de destinos crea carpetas vacías espurias
...
### A1-5 · MENOR — ✅ CORREGIDO (Cursor) — `evaluar` reemplaza el error real por el motivo genérico
...
### A1-6 · MENOR — ✅ CORREGIDO (Cursor) — `evaluar` no cubre `KeyError`: un Efth con dims no estándar tumba todo el tablero
...
### A1-7 · MENOR — ✅ CORREGIDO (Cursor) — JONSWAP no valida Hs
...
### A2-4 · MEDIO — ✅ CORREGIDO (Cursor) — Credenciales: se exige formato `UID:API-KEY` que el CDS nuevo ya no usa
...
### A2-5 · MENOR — ✅ CORREGIDO (Cursor) — `_cache_utilizable` es código muerto
...
### A2-6 · MENOR — ✅ CORREGIDO (Cursor) — `descargar_serie` devuelve un Dataset lazy con el archivo de caché abierto
...
### A2-7 · MENOR — ✅ CORREGIDO (Cursor) — `validar_rango_fechas` no rechaza fechas futuras
...
### A3-1 · MEDIO — ✅ CORREGIDO (Cursor) — Partición sobre espectros SWAN: espectro por-grado integrado con ddir en radianes → Hs de familias ~7,6× menor
...
### A3-2 · MENOR — ✅ CORREGIDO (Cursor) — `swan_disponible()` acepta `swan` a secas, pero `correr_caso` solo lanza `swanrun`
...
### A3-3 · MENOR — ✅ CORREGIDO (Cursor) — `BREAKING CON 1.0 0.29` fijo en todos los `.swn` generados
...
### A4-1 · MEDIO — ✅ CORREGIDO (Cursor) — `_error_tarea` reduce los `RuntimeError` informativos a la palabra "RuntimeError"
...
### A4-2 · MENOR — ✅ CORREGIDO (Cursor) — Borrar caché ERA5 puede fallar con un error genérico si la serie está abierta
...
### A4-3 · MENOR — ✅ CORREGIDO (Cursor) — Las tareas en hilo no-SWAN no se pueden cancelar
...
### A5-1 · MENOR — ✅ CORREGIDO (Cursor) — En las vistas fuera de wizard los errores no se muestran en ninguna parte
...
### A5-2 · MENOR — ✅ CORREGIDO (Cursor) — Excepciones no controladas del puente se tragan como *unhandled rejection*
...
### A5-3 · MENOR — ✅ CORREGIDO (Cursor) — Las instrucciones de credenciales fijan el formato antiguo `UID:API-KEY`
...
### A6-1 · CRÍTICO — ✅ CORREGIDO (Cursor) — La app instalada con el `.exe` en Program Files no puede funcionar sin elevación

> **Código listo (2026-07-06, Cursor):** instalación per-user (`PrivilegesRequired=lowest`,
> `{localappdata}`), fallback de `rutas.py`/`config.py`/`bootstrap_windows.ps1` a
> `%LOCALAPPDATA%`. **Pendiente prueba manual:** recompilar `empaquetar_instalador.bat`
> y probar el Setup v1.0.1 en máquina sin permisos de admin antes de re-publicar el Release.
...
### A6-2 · MENOR — ✅ CORREGIDO (Cursor) — El binario del instalador está commiteado en el repo público
...
### A7-1 · MEDIO — ✅ CORREGIDO (Claude Code) — Un test de regresión fija el comportamiento erróneo de A2-1
...
### A7-2 · MENOR — ✅ CORREGIDO (Cursor) — `test_motor_web.py` quedó fuera del comando oficial de la suite
...
### A7-3 · MENOR — ✅ CORREGIDO (Cursor) — Los tests con datos reales nunca corren en este equipo pese a que los datos existen
...
| A6-1 — Instalador Windows en Program Files | Instalación per-user + fallback LOCALAPPDATA; pendiente probar Setup v1.0.1 en máquina real | `9eabaca` | 159 passed, 8 skipped |
